...
class RedisClient {
  ...
  private async createIndexes(): Promise<void> {
    try {
      ...
      await this.client.call('FT.CREATE', 'idx:users',
        'ON', 'JSON',
        'PREFIX', '1', 'user:',
        'SCHEMA',
        '$.inboxId', 'AS', 'inboxId', 'TEXT',
        '$.email', 'AS', 'email', 'TEXT',
        '$.name', 'AS', 'name', 'TEXT',
        '$.shippingAddress.city', 'AS', 'city', 'TEXT',
        '$.shippingAddress.state', 'AS', 'state', 'TEXT',
        '$.isComplete', 'AS', 'complete', 'TAG',
        '$.walletAddress', 'AS', 'walletAddress', 'TEXT'
      );
    ...
    try {
      ...
      await this.client.call('FT.CREATE', 'idx:orders',
        'ON', 'JSON',
        'PREFIX', '1', 'order:',
        'SCHEMA',
        '$.id', 'AS', 'orderId', 'TEXT',
  ...
  async appendUserOrder(inboxId: string, order: any): Promise<void> {
    const key = `user:${inboxId}`;
    await this.client.call('JSON.ARRAPPEND', key, '$.orderHistory', JSON.stringify(order));
  ...
  async cacheConversationState(inboxId: string, state: any, ttl = 3600): Promise<void> {
  ...
  async searchUsers(query: string): Promise<UserProfile[]> {
    try {
      const result = await this.client.call('FT.SEARCH', 'idx:users', query, 'LIMIT', '0', '50');
...
export const saveUserOrderId = async ({ profile, order }: { profile: UserProfile; order: any }): Promise<void> => {
  await redisClient.connect();
